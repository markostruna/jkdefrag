...
void SlowDown(struct DefragDataStruct *Data) {
	...
	/* Save the current wall time, so next time we can calculate the time spent in
	the program. */
	_ftime64_s(&Time);
...
/* Return pointer to the first item in the tree (the first file on the
volume). */
...
/* Return pointer to the last item in the tree (the last file on the
volume). */
...
HANDLE OpenItemHandle(struct DefragDataStruct *Data, struct ItemStruct *Item) {
	...
	m_jkGui->ShowDebug(4,NULL,Data->DebugMsg[15],Item->LongPath,ErrorString);
...
int GetFragments(struct DefragDataStruct *Data, struct ItemStruct *Item, HANDLE FileHandle) {
	...
	m_jkGui->ShowDebug(4,NULL,Data->DebugMsg[10],Item->LongPath);
	...
	do {
		...
		if (MaxLoop <= 0)
		{
			m_jkGui->ShowDebug(2,NULL,L"FSCTL_GET_RETRIEVAL_POINTERS error: Infinite loop");
		...
		for (i = 0; i < ExtentData.ExtentCount; i++)
		{
			...
			if (ExtentData.Extents[i].Lcn != VIRTUALFRAGMENT)
			{
				...
				m_jkGui->ShowDebug(4,NULL,Data->DebugMsg[11],ExtentData.Extents[i].Lcn,Vcn,
					ExtentData.Extents[i].NextVcn);
			}
			else
			{
				/* "Extent (virtual): Vcn=%I64u, NextVcn=%I64u" */
				m_jkGui->ShowDebug(4,NULL,Data->DebugMsg[46],Vcn,ExtentData.Extents[i].NextVcn);
			}
	...
	if ((ErrorCode != NO_ERROR) && (ErrorCode != ERROR_HANDLE_EOF))
	{
		...
		m_jkGui->ShowDebug(3,NULL,Data->DebugMsg[43],Item->LongPath,ErrorString);
...
int IsFragmented(struct ItemStruct *Item, ULONG64 Offset, ULONG64 Size) {
	...
	Fragment = Item->Fragments;
	while (Fragment != NULL) {
		/* Virtual fragments do not occupy space on disk and do not count as
		fragments. */
		if (Fragment->Lcn != VIRTUALFRAGMENT) {
			...
			if ((NextLcn != 0) && (Fragment->Lcn != NextLcn)) {
				...
				if ((FragmentBegin > Offset) ||
					...
					(FragmentEnd - 1 < Offset + Size - 1))) return(YES);
	...
	BOOL IgnoreMftExcludes) {
		...
		do
		{
			...
			if ((ErrorCode != NO_ERROR) && (ErrorCode != ERROR_MORE_DATA))
			{
				...
				m_jkGui->ShowDebug(1,NULL,Data->DebugMsg[12],s1);
			...
			while ((Index < IndexMax) && (Lcn < MaximumLcn))
			{
				if (Lcn >= MinimumLcn)
				{
					...
					if ((PrevInUse == 0) && (InUse != 0))
					{
						...
						m_jkGui->ShowDebug(6,NULL,Data->DebugMsg[13],ClusterStart,Lcn - ClusterStart);
...
void CalculateZones(struct DefragDataStruct *Data) {
	...
	for (Iterate = 1; Iterate <= 10; Iterate++) {
		...
		/* Show debug info. */
		m_jkGui->ShowDebug(4,NULL,L"Zone calculation, iteration %u: 0 - %I64d - %I64d - %I64d",Iterate,
...
struct ItemStruct *Item,
	...
	ULONG64 Size) {                     /* Number of clusters to be moved. */
		...
		m_jkGui->ShowMove(Item,MoveParams.ClusterCount,Lcn,NewLcn,MoveParams.StartingVcn.QuadPart);
		...
		m_jkGui->DrawCluster(Data,NewLcn,NewLcn + Size,JKDefragStruct::COLORBUSY);
		...
		m_jkGui->DrawCluster(Data,NewLcn,NewLcn + Size,JKDefragStruct::COLOREMPTY);
...
struct ItemStruct *Item,
	...
	ULONG64 Size) {                     /* Number of clusters to be moved. */
		...
		/* Walk through the fragments of the item and move them one by one to the
		new location. */
		ErrorCode = NO_ERROR;
		Vcn = 0;
		RealVcn = 0;
...
//		}
		...
		{
			m_jkGui->ShowDebug(3,Item,ErrorString);
...
struct ItemStruct *Item,
	...
	int Direction) {                          /* 0: move up, 1: move down. */
		...
		m_jkGui->ShowDebug(3,Item,Data->DebugMsg[42]);
		...
		if (IsFragmented(Item,Offset,Size) == YES)
		{
			...
			m_jkGui->ShowDebug(3,Item,Data->DebugMsg[45]);

			return(NO);
		}

		m_jkGui->ShowDebug(3,Item,L"");

		/* Add the size of the item to the width of the progress bar, we have more work
		to do. */
		Data->PhaseTodo = Data->PhaseTodo + Size;
...
struct ItemStruct *Item,
	...
	int Direction) {                          /* 0: move up, 1: move down. */
		...
		/* If the Item is Unmovable, Excluded, or has zero size then we cannot move
		it. */
...
*/
struct ItemStruct *FindHighestItem(
struct DefragDataStruct *Data,
	ULONG64 ClusterStart,
	ULONG64 ClusterEnd,
	int Direction,
	int Zone) {
		...
		m_jkGui->ShowDebug(5,NULL,L"Looking for highest-fit %I64d[%I64d]",
...
Zone=3           Search all items.
*/
struct ItemStruct *FindBestItem(
struct DefragDataStruct *Data,
	ULONG64 ClusterStart,
	ULONG64 ClusterEnd,
	int Direction,
	int Zone) {
		...
		m_jkGui->ShowDebug(5,NULL,L"Looking for perfect fit %I64d[%I64d]",
			ClusterStart,ClusterEnd - ClusterStart);
			...
			Item = TreeNextPrev(Item,Direction)) {
				...
				if (((Direction == 1) && (ItemLcn < ClusterEnd)) ||
					((Direction == 0) && (ItemLcn > ClusterEnd))) {
						...
						{
							m_jkGui->ShowDebug(5,NULL,L"No perfect fit found, the total size of all the items above the gap is less than the size of the gap.");
						...
						{
							m_jkGui->ShowDebug(5,NULL,L"No perfect fit found, out of time.");
				...
				{
					m_jkGui->ShowDebug(5,NULL,L"Perfect fit found.");
		...
		m_jkGui->ShowDebug(5,NULL,L"No perfect fit found, all items above the gap are bigger than the gap.");
...
void CompareItems(struct DefragDataStruct *Data, struct ItemStruct *Item) {
	...
	m_jkGui->ShowDebug(0,NULL,L"%I64u %s",GetItemLcn(Item),Item->LongFilename);
	...
	if (FileHandle == INVALID_HANDLE_VALUE)
	{
		...
		m_jkGui->ShowDebug(0,NULL,L"  Could not open: %s",ErrorString);
	...
	if (GetFileInformationByHandle(FileHandle,&FileInformation) != 0)
	{
		...
		{
			m_jkGui->ShowDebug(0,NULL,L"  Different CreationTime %I64u <> %I64u = %I64u",
		...
		{
			m_jkGui->ShowDebug(0,NULL,L"  Different LastAccessTime %I64u <> %I64u = %I64u",
	...
	do {
		...
		if (MaxLoop <= 0)
		{
			m_jkGui->ShowDebug(0,NULL,L"  FSCTL_GET_RETRIEVAL_POINTERS error: Infinite loop");
		...
		for (i = 0; i < ExtentData.ExtentCount; i++) {
			...
			if (Fragment == NULL)
			{
				m_jkGui->ShowDebug(0,NULL,L"  Extra fragment in FSCTL_GET_RETRIEVAL_POINTERS");
			}
			else
			{
				if (Fragment->Lcn != ExtentData.Extents[i].Lcn)
				{
					m_jkGui->ShowDebug(0,NULL,L"  Different LCN in fragment: %I64u <> %I64u",
						Fragment->Lcn,ExtentData.Extents[i].Lcn);
				}

				if (Fragment->NextVcn != ExtentData.Extents[i].NextVcn)
				{
					m_jkGui->ShowDebug(0,NULL,L"  Different NextVcn in fragment: %I64u <> %I64u",
	...
	if ((ErrorCode != NO_ERROR) && (ErrorCode != ERROR_HANDLE_EOF))
	{
		...
		m_jkGui->ShowDebug(0,Item,L"  Error while processing clustermap: %s",ErrorString);
	...
	if (Fragment != NULL)
	{
		m_jkGui->ShowDebug(0,NULL,L"  Extra fragment from MFT");
	}

	if (Item->Clusters != Clusters)
	{
		m_jkGui->ShowDebug(0,NULL,L"  Different cluster count: %I64u <> %I64u",
...
void ScanDir(struct DefragDataStruct *Data, WCHAR *Mask, struct ItemStruct *ParentDirectory) {
	...
	m_jkGui->ShowDebug(3,NULL,Data->DebugMsg[23],Mask);
...
void AnalyzeVolume(struct DefragDataStruct *Data) {
	...
	if ((Result == FALSE) && (*Data->Running == RUNNING)) {
		m_jkGui->ShowDebug(0,NULL,L"This is not a FAT or NTFS disk, using the slow scanner.");

		/* Setup the width of the progress bar. */
		Data->PhaseTodo = Data->TotalClusters - Data->CountFreeClusters;
		for (i = 0; i < 3; i++) {
			Data->PhaseTodo = Data->PhaseTodo -
				(Data->MftExcludes[i].End - Data->MftExcludes[i].Start);
		}
	...
	m_jkGui->DrawCluster(Data,0,0,0);
...
//		if (*Data->RedrawScreen == 1) m_jkGui->ShowDiskmap(Data);
		...
		*/
		...
		if (Data->PhaseDone % 10000 == 0) m_jkGui->DrawCluster(Data,0,0,0);
	}
...
void Fixup(struct DefragDataStruct *Data) {
	...
	while ((NextItem != NULL) && (*Data->Running == RUNNING)) {
		...
		if (IsFragmented(Item,0,Item->Clusters) == YES) {
			/* "I am fragmented." */
			m_jkGui->ShowDebug(4,Item,Data->DebugMsg[53]);
			MoveMe = YES;
		}
		if ((MoveMe == NO) &&
		...
		if (Item->Clusters > GapEnd[FileZone] - GapBegin[FileZone]) {
			...
			if (Result == NO) {
				...
				m_jkGui->ShowDebug(2,Item,Data->DebugMsg[25],GetItemLcn(Item),Item->Clusters);
	...
	while ((NextItem != NULL) && (*Data->Running == RUNNING)) {
		...
		if (Result == NO) {
			...
			if (Result == NO) {
				...
				m_jkGui->ShowDebug(2,Item,Data->DebugMsg[44]);
				return;
			}
		}
...
/* Fill all the gaps at the beginning of the disk with fragments from the
files above. */
void ForcedFill(struct DefragDataStruct *Data) {
	...
	while (*Data->Running == RUNNING) {
		...
		Result = MoveItem(Data,HighestItem,GapBegin,HighestVcn + HighestSize - Clusters,
			Clusters,0);
...
void Vacate(struct DefragDataStruct *Data, ULONG64 Lcn, ULONG64 Clusters, BOOL IgnoreMftExcludes) {
	...
	m_jkGui->ShowDebug(5,NULL,L"Vacating %I64u clusters starting at LCN=%I64u",Clusters,Lcn);

	/* Sanity check. */
	if (Lcn >= Data->TotalClusters) {
		m_jkGui->ShowDebug(1,NULL,L"Error: trying to vacate an area beyond the end of the disk.");
		return;
	}
	...
	m_jkGui->ShowDebug(5,NULL,L"MoveTo = %I64u",MoveTo);
	...
	while (*Data->Running == RUNNING) {
		...
		if (BiggerItem == NULL) {
			m_jkGui->ShowDebug(5,NULL,L"No data found above LCN=%I64u",Lcn);
			return;
		}
		m_jkGui->ShowDebug(5,NULL,L"Data found at LCN=%I64u, %s",BiggerBegin,BiggerItem->LongPath);

		/* Find the first gap above the Lcn. */
		Result = FindGap(Data,Lcn,0,0,YES,NO,&TestGapBegin,&TestGapEnd,IgnoreMftExcludes);
		if (Result == NO) {
			m_jkGui->ShowDebug(5,NULL,L"No gaps found above LCN=%I64u",Lcn);
			return;
		}
		...
		if (TestGapEnd < BiggerBegin) {
			m_jkGui->ShowDebug(5,NULL,L"Cannot enlarge the gap from %I64u to %I64u (%I64u clusters) any further.",
		...
		if ((TestGapEnd == BiggerBegin) && (TestGapEnd - TestGapBegin >= Clusters)) {
			m_jkGui->ShowDebug(5,NULL,L"Finished vacating, the gap from %I64u to %I64u (%I64u clusters) is now bigger than %I64u clusters.",
		...
		if (Lcn >= MoveTo) {
			m_jkGui->ShowDebug(5,NULL,L"Stopping vacate because of possible worm.");
			return;
		}
		...
		if (BiggerEnd - BiggerBegin >= MoveGapEnd - MoveGapBegin) {
			...
			if ((MoveTo < Data->TotalClusters) && (MoveTo >= BiggerEnd)) {
				m_jkGui->ShowDebug(5,NULL,L"Finding gap above MoveTo=%I64u",MoveTo);
			...
			above the item. */
			if (Result == NO) {
				m_jkGui->ShowDebug(5,NULL,L"Finding gap from end of disk above BiggerEnd=%I64u",BiggerEnd);
			...
			if (Result == NO) {
				m_jkGui->ShowDebug(5,NULL,L"No gap found.");
				return;
			}
		}
...
void OptimizeSort(struct DefragDataStruct *Data, int SortField) {
	...
	for (Data->Zone = 0; Data->Zone < 3; Data->Zone++) {
		...
		while (*Data->Running == RUNNING) {
			...
			if (Item == NULL) {
				m_jkGui->ShowDebug(2,NULL,L"Finished sorting zone %u.",Data->Zone+1);
				break;
			}
			PreviousItem = Item;
			...
			while ((*Data->Running == RUNNING) &&
				...
				(Item->Unmovable == NO)) {
					...
					if (ClustersDone > 0) {
						m_jkGui->ShowDebug(5,NULL,L"Item partially placed, %I64u clusters more to do",
...
void MoveMftToBeginOfDisk(struct DefragDataStruct *Data) {
	...
	m_jkGui->ShowDebug(2,NULL,L"Moving the MFT to the beginning of the volume.");

	/* Exit if this is not an NTFS disk. */
	if (Data->Disk.Type != NTFS) {
		m_jkGui->ShowDebug(5,NULL,L"Cannot move the MFT because this is not an NTFS disk.");
		return;
	}
	...
	if ((GetVersionEx(&OsVersion) != 0) && (OsVersion.dwMajorVersion < 6)) {
		m_jkGui->ShowDebug(5,NULL,L"Cannot move the MFT because it is not supported by this version of Windows.");
		return;
	}
	...
	if (Item == NULL) {
		m_jkGui->ShowDebug(5,NULL,L"Cannot move the MFT because I cannot find it.");
		return;
	}
	...
	while ((*Data->Running == RUNNING) && (ClustersDone < Item->Clusters)) {
		...
		if (ClustersDone > Data->Disk.MftLockedClusters) {
			m_jkGui->ShowDebug(5,NULL,L"Partially placed, %I64u clusters more to do",
				Item->Clusters - ClustersDone);
		}
	...
	for (Zone = 0; Zone < 3; Zone++) {
		...
		while (*Data->Running == RUNNING) {
			...
			if (GapBegin < GapEnd) {
				...
				m_jkGui->ShowDebug(5,NULL,Data->DebugMsg[28],GapBegin,GapEnd - GapBegin);
				GapBegin = GapEnd;
				Retry = 0;
			}
		}
...
void DefragOnePath(struct DefragDataStruct *Data, WCHAR *Path, int Mode) {
	...
	if (Data->Excludes != NULL) {
		...
		if (Data->Excludes[i] != NULL) {
			...
			m_jkGui->ShowDebug(0,NULL,Data->DebugMsg[47],Path,Data->Excludes[i]);
			return;
		}
	}

	/* Clear the screen and show "Processing '%s'" message. */
	m_jkGui->ClearScreen(Data->DebugMsg[14],Path);
	...
	*/
		...
		(LookupPrivilegeValue(0,SE_BACKUP_NAME,&TakeOwnershipValue) != 0)) {
			...
			if (AdjustTokenPrivileges(ProcessTokenHandle,FALSE,&TokenPrivileges,
				sizeof(TOKEN_PRIVILEGES),0,0) == FALSE) {
					m_jkGui->ShowDebug(3,NULL,L"Info: could not elevate to SeBackupPrivilege.");
			}
	} else {
		m_jkGui->ShowDebug(3,NULL,L"Info: could not elevate to SeBackupPrivilege.");
	}
	...
	if (Result == 0) {
		if (wcslen(Data->Disk.MountPointSlash) > 52 - 1 - 4) {
			...
			SystemErrorStr(GetLastError(),s1,BUFSIZ);
			m_jkGui->ShowDebug(0,NULL,Data->DebugMsg[40],Data->Disk.MountPointSlash,s1);
			free(Data->Disk.MountPoint);
	...
	if ((Result == 0) && (Fin != NULL)) {
		...
		if ((fread(&w,4,1,Fin) == 1) && (w != 0)) {
			m_jkGui->ShowDebug(0,NULL,L"Will not process this disk, it contains hybernated data.");
			free(Data->Disk.MountPoint);
			free(Data->Disk.MountPointSlash);
			free(p1);
			return;
	...
	m_jkGui->ShowDebug(0,NULL,Data->DebugMsg[29],Data->Disk.VolumeName,Data->Disk.MountPoint);
	...
	if (Data->Disk.VolumeHandle == INVALID_HANDLE_VALUE) {
		SystemErrorStr(GetLastError(),s1,BUFSIZ);
		m_jkGui->ShowDebug(1,NULL,L"Cannot open volume '%s' at mountpoint '%s': %s",
...
//	long koko = FSCTL_GET_VOLUME_BITMAP;
	...
	if ((ErrorCode != NO_ERROR) && (ErrorCode != ERROR_MORE_DATA)) {
		...
		m_jkGui->ShowDebug(0,NULL,Data->DebugMsg[32],Data->Disk.VolumeName,Data->Disk.MountPoint);
	...
	*/
	...
	if (ErrorCode != 0) {
		...
		m_jkGui->ShowDebug(3,NULL,Data->DebugMsg[33],
		...
		m_jkGui->ShowDebug(3,NULL,Data->DebugMsg[34],0,Data->MftExcludes[0].Start,0,Data->MftExcludes[0].End);
		m_jkGui->ShowDebug(3,NULL,Data->DebugMsg[34],1,Data->MftExcludes[1].Start,1,Data->MftExcludes[1].End);
		m_jkGui->ShowDebug(3,NULL,Data->DebugMsg[34],2,Data->MftExcludes[2].Start,2,Data->MftExcludes[2].End);
	}
...
void DefragMountpoints(struct DefragDataStruct *Data, WCHAR *MountPoint, int Mode) {
	...
	m_jkGui->ClearScreen(Data->DebugMsg[37],MountPoint);

	/* Return if this is not a fixed disk. */
	DriveType = GetDriveTypeW(MountPoint);
	if (DriveType != DRIVE_FIXED) {
		if (DriveType == DRIVE_UNKNOWN) {
			m_jkGui->ClearScreen(L"Ignoring volume '%s' because the drive type cannot be determined.",MountPoint);
		}
		if (DriveType == DRIVE_NO_ROOT_DIR) {
			m_jkGui->ClearScreen(L"Ignoring volume '%s' because there is no volume mounted.",MountPoint);
		}
		if (DriveType == DRIVE_REMOVABLE) {
			m_jkGui->ClearScreen(L"Ignoring volume '%s' because it has removable media.",MountPoint);
		}
		if (DriveType == DRIVE_REMOTE) {
			m_jkGui->ClearScreen(L"Ignoring volume '%s' because it is a remote (network) drive.",MountPoint);
		}
		if (DriveType == DRIVE_CDROM) {
			m_jkGui->ClearScreen(L"Ignoring volume '%s' because it is a CD-ROM drive.",MountPoint);
		}
		if (DriveType == DRIVE_RAMDISK) {
			m_jkGui->ClearScreen(L"Ignoring volume '%s' because it is a RAM disk.",MountPoint);
		}
		return;
	}
	...
	if (Result == 0) {
		...
		if (ErrorCode == 3) {
			...
			m_jkGui->ShowDebug(0,NULL,Data->DebugMsg[57],MountPoint);
		} else {
			/* "Cannot find volume name for mountpoint: %s" */
			SystemErrorStr(ErrorCode,s1,BUFSIZ);
			m_jkGui->ShowDebug(0,NULL,Data->DebugMsg[40],MountPoint,s1);
		}
		return;
	}
	...
	if ((FileSystemFlags & FILE_READ_ONLY_VOLUME) != 0) {
		...
		m_jkGui->ClearScreen(Data->DebugMsg[36],MountPoint);
		return;
	}
	...
	if (DeviceIoControl(VolumeHandle,FSCTL_IS_VOLUME_MOUNTED,NULL,0,NULL,0,&w,NULL) == 0) {
		...
		m_jkGui->ShowDebug(0,NULL,Data->DebugMsg[31],VolumeName,MountPoint);
...
*/
	...
	if (Data.UseDefaultSpaceHogs == TRUE) {
		...
		if (Data.UseLastAccessTime == TRUE) {
			m_jkGui->ShowDebug(1,NULL,L"NtfsDisableLastAccessUpdate is inactive, using LastAccessTime for SpaceHogs.");
		} else {
			m_jkGui->ShowDebug(1,NULL,L"NtfsDisableLastAccessUpdate is active, ignoring LastAccessTime for SpaceHogs.");
		}
	}
	...
	if ((Path != NULL) && (*Path != 0)) {
		...
		if (Drives != NULL) {
			...
			if (DrivesSize == 0) {
				...
				SystemErrorStr(GetLastError(),s1,BUFSIZ);
				m_jkGui->ShowDebug(1,NULL,Data.DebugMsg[39],s1);
			} else {
				Drive = Drives;
				while (*Drive != '\0') {
		...
		m_jkGui->ClearScreen(Data.DebugMsg[38]);
	}
