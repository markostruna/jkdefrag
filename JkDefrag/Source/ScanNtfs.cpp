...
struct NtfsDiskInfoStruct *DiskInfo,
	...
	ULONG64 WantedLength) {            /* Number of bytes to read. */
		...
		while (RunData[Index] != 0) {
			...
			Result = ReadFile(Data->Disk.VolumeHandle,&Buffer[ExtentVcn - Offset],
				(DWORD)ExtentLength,&BytesRead,&gOverlapped);
			if (Result == 0) {
	...
	ULONG64 Bytes) {
		...
		if (RunData != NULL) while (RunData[Index] != 0) {
			...
			if (RunOffset.Value != 0) {
				m_jkGui->ShowDebug(6,NULL,L"    Extent: Lcn=%I64u, Vcn=%I64u, NextVcn=%I64u",
					Lcn,Vcn - RunLength.Value,Vcn);
			} else {
				m_jkGui->ShowDebug(6,NULL,L"    Extent (virtual): Vcn=%I64u, NextVcn=%I64u",
...
struct InodeDataStruct *InodeData,
	...
	int Depth) {
		...
		m_jkGui->ShowDebug(6,NULL,L"    Processing AttributeList for Inode %I64u, %u bytes",
			InodeData->Inode,BufLength);
		...
		for (AttributeOffset = 0; AttributeOffset < BufLength; AttributeOffset = AttributeOffset + Attribute->Length) {
			...
			*/
			...
			Vcn = 0;
			RealVcn = 0;
			RefInodeVcn = RefInode * DiskInfo->BytesPerMftRecord /
				(DiskInfo->BytesPerSector * DiskInfo->SectorsPerCluster);
			...
			gOverlapped.OffsetHigh = Trans.HighPart;
			gOverlapped.hEvent     = NULL;
			Result = ReadFile(Data->Disk.VolumeHandle,Buffer2,
				(DWORD)DiskInfo->BytesPerMftRecord,&BytesRead,&gOverlapped);
			...
			m_jkGui->ShowDebug(6,NULL,L"      Processing Inode %I64u Instance %u",RefInode,
				Attribute->Instance);
			...
			m_jkGui->ShowDebug(6,NULL,L"      Finished processing Inode %I64u Instance %u",RefInode,
				Attribute->Instance);
			free(Buffer2);
		}
}
	...
	int Depth) {
		...
		for (AttributeOffset = 0; AttributeOffset < BufLength; AttributeOffset = AttributeOffset + Attribute->Length) {
			...
			if ((AttributeOffset + 4 <= BufLength) &&
				(*(ULONG *)Attribute == 0xFFFFFFFF)) break;

			/* Sanity check. */
			if ((AttributeOffset + 4 > BufLength) ||
				(Attribute->Length < 3) ||
				(AttributeOffset + Attribute->Length > BufLength)) {
					m_jkGui->ShowDebug(2,NULL,L"Error: attribute in Inode %I64u is bigger than the data, the MFT may be corrupt.",
						InodeData->Inode);
			...
			/* Show debug message. */
			m_jkGui->ShowDebug(6,NULL,L"  Attribute %u: %s",Attribute->AttributeNumber,
				StreamTypeNames(Attribute->AttributeType));

			if (Attribute->Nonresident == 0) {
				ResidentAttribute = (struct RESIDENT_ATTRIBUTE *)Attribute;

				/* The AttributeFileName (0x30) contains the filename and the link to the parent
				directory. */
				if (Attribute->AttributeType == AttributeFileName) {
					FileNameAttribute = (struct FILENAME_ATTRIBUTE *)&Buffer[AttributeOffset +
				...
				if (InodeData->Inode == 0) {
					...
					if ((Attribute->AttributeType == AttributeBitmap) &&
...
struct FragmentListStruct **MftBitmapFragments,
	...
	ULONG64 BufLength) {
		...
		BaseInode = (ULONG64)FileRecordHeader->BaseFileRecord.InodeNumberLowPart +
			((ULONG64)FileRecordHeader->BaseFileRecord.InodeNumberHighPart << 32);
		if (BaseInode != 0) {
			m_jkGui->ShowDebug(6,NULL,L"Ignoring Inode %I64u, it's an extension of Inode %I64u",
				InodeNumber,BaseInode);
			return(TRUE);
		}
		...
		if (InodeData.Directory == YES) {
			TranslateRundataToFragmentlist(Data,&InodeData,L"$I30",AttributeIndexAllocation,
				NULL,0,0,0);
		}
		...
		do {
			...
			Item->LongFilename = ConstructStreamName(InodeData.LongFilename,InodeData.ShortFilename,
				Stream);
			Item->LongPath = NULL;
			Item->ShortFilename = ConstructStreamName(InodeData.ShortFilename,InodeData.LongFilename,
				Stream);
			Item->ShortPath = NULL;
...
BOOL AnalyzeNtfsVolume(struct DefragDataStruct *Data) {
	...
	if (ClustersPerMftRecord >= 128) {
		...
		DiskInfo.BytesPerMftRecord = ClustersPerMftRecord * DiskInfo.BytesPerSector *
	...
	the MFT. */
	Trans.QuadPart         = DiskInfo.MftStartLcn * DiskInfo.BytesPerSector *
		DiskInfo.SectorsPerCluster;
	gOverlapped.Offset     = Trans.LowPart;
	gOverlapped.OffsetHigh = Trans.HighPart;
	gOverlapped.hEvent     = NULL;
	Result = ReadFile(Data->Disk.VolumeHandle,Buffer,(DWORD)DiskInfo.BytesPerMftRecord,
	...
	/* Fixup the raw data from disk. This will also test if it's a valid $MFT
	record. */
	...
	m_jkGui->ShowDebug(6,NULL,L"MftDataBytes = %I64u, MftBitmapBytes = %I64u",
	...
	for (Fragment = MftBitmapFragments; Fragment != NULL; Fragment = Fragment->Next) {
		if (Fragment->Lcn != VIRTUALFRAGMENT) {
			...
			Result = ReadFile(Data->Disk.VolumeHandle,
				...
				&BytesRead,&gOverlapped);
			if ((Result == 0) ||
				(BytesRead != (Fragment->NextVcn - Vcn) * DiskInfo.BytesPerSector * DiskInfo.SectorsPerCluster)) {
					SystemErrorStr(GetLastError(),s1,BUFSIZ);
